...
export const usePhotos = () => {
  ...
  const loadFolders = useCallback(async () => {
    try {
      ...
      if (response.error) {
        ...
        const apiResponse = response.data as { data?: string[] } & Record<
          string,
          any
        >;
    ...
    async (folderName: string) => {
      try {
        ...
        if (response.error) {
          ...
          const apiResponse = response.data as { data?: any[] } & Record<
            string,
            any
          >;
    ...
    async (
      ...
      try {
        ...
        if (response.error) {
          ...
          toast({
            title: "Success",
            description: `Photo "${newPhoto.filename}" uploaded successfully`,
          });
          ...
          // Refresh folders to ensure they're up to date
          await loadFolders();

          // If we're in a specific folder, refresh its photos
  ...
  const deletePhoto = useCallback(
    async (photoId: number, folderName?: string) => {
      try {
        setLoading(true);
        setError(null);
